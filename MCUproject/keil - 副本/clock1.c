...
#define PA8255 0xff21  //外部82C55 b口
#define PB8255 0xff22 //外部82C55 C口
#define COM8255 0xff20 //82C55 控制口
...
{
   XBYTE[PA8255] = 0x3E;  //显示秒个位 111110
   XBYTE[PB8255] = table[sec1%10];                   //显示秒个位
   delayms(3);
	 
   XBYTE[PA8255]=0x3D;//111101
   XBYTE[PB8255]=table[sec1/10];                  //显示秒十位
   delayms(3);
/*
   P2=0x03;                                  //显示横线
   P0=0xbf;
   delayms(5);
*/
   XBYTE[PA8255]=0x3B; //111011                                         //显示分个位
   XBYTE[PB8255]=table[min1%10]&0x7f;
   delayms(3);

   XBYTE[PA8255]=0x37; //110111                                         //显示分十位
...
/*
   ...
   delayms(5);
*/
   XBYTE[PA8255]=0x2F;         //101111                                  //显示时个位
   XBYTE[PB8255]=table[hour1%10]&0x7f;    //八段数码管显示小数点仅需要在段选码后 + &0x7F !!!
   delayms(3);
